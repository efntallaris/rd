...
void traverse_print_slot_blocks_filename(int slot, const char *filename)
{
 ...
 //                cur_block->segments_used, cur_block->bytes_free);
    ...
    size_t total_blocks = total_used_bytes/slots_bytes_free;
    // int total_blocks = 2 ;
    ...
    fprintf(file, "Total Blocks:%d, Total Segments:%zu, Last Slot unused bytes:%zu. Actual bytes free per block:%zu, total bytes of slot:%zu\n", total_blocks, total_segments, rest_free_bytes_on_last_slot, slots_bytes_free, total_used_bytes);
    fprintf(file, "\n");
...
int load_slot_from_file(int slot, char *fileName)
{
    ...
    int bytes_read = 0;
    char *tmp_block = (char *) malloc(BLOCK_SIZE_BYTES);
