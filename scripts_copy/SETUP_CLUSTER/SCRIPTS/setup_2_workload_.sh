...
for redis_instance in "${!redis_ycsb_instances[@]}"; do
    ...
    for i in "${!nodeInstance[@]}"; do
		...
		sudo ./ycsb.sh run redis -p "redis.host=${MASTER_HOST}" -p "redis.port=${MASTER_PORT}" -p "redis.cluster=true" -P ${REDIS_WORKLOAD} -p status.interval=1 -s  -p \measurementtype=timeseries -p measurementtype=timeseries -p redis.timeout=10000 -threads 200 >> ${YCSB_LOG_FILENAME}_${info[0]} 2>&1 &
EOF
2>&1)
    echo "$tko"
    done
