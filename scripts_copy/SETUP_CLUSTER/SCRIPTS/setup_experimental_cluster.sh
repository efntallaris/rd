...
REDIS_DATA_VALIDATOR_PATH="/mnt/stratos/ycsb-0.5.0/new_modded_ycsb/newer/RedisDataValidator"

REDIS_HOST="127.0.0.1"
REDIS_PORT="8000"
REDIS_WORKLOAD="/mnt/stratos/modded_redis/workloads/experimental_workload"
...
sleep 5
cd $YCSB_DIR
ycsbCommand=$(sudo ./ycsb load redis -p "redis.host=${REDIS_HOST}" -p "redis.port=${REDIS_PORT}" -p "redis.cluster=true" -P ${REDIS_WORKLOAD} -p "redis.logfile=/home/entallaris/dataIsert.log" -threads 1)
echo ${ycsbCommand}
ycsbCommand=$(sudo ./ycsb run redis -p "redis.host=${REDIS_HOST}" -p "redis.port=${REDIS_PORT}" -p "redis.cluster=true" -P ${REDIS_WORKLOAD} -p "redis.logfile=/home/entallaris/dataRead.log" -threads 1)
echo ${ycsbCommand}
#cd ${REDIS_DATA_VALIDATOR_PATH}
#redisDataValidatorCommand=$(sudo java -jar ./target/data_validator-0.1.jar -h ${REDIS_HOST} -p ${REDIS_PORT} -d /home/entallaris/data.log12)
#echo ${redisDataValidatorCommand}
sleep 10
