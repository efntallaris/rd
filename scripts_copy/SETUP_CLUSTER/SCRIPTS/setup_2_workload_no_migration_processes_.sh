...
REDIS_PORT="8000"
REDIS_WORKLOAD_PATH="../../workloads/"
REDIS_WORKLOAD_NAME=$1
REDIS_NUMBER_OF_PROCESSES=$2
REDIS_WORKLOAD=${REDIS_WORKLOAD_PATH}${REDIS_WORKLOAD_NAME}
YCSB_LOG_FILENAME="/home/entallaris/ycsb_output_${REDIS_WORKLOAD_NAME}"
...
echo $REDIS_WORKLOAD
echo "here"
if [[ $REDIS_WORKLOAD == *"read"* ]]; then
    echo "ok"
fi

if [[ $REDIS_NUMBER_OF_PROCESSES -gt 0 ]]; then
    ycsbCommand=$(sudo ./ycsb.sh run redis -p "redis.host=${REDIS_HOST}" -p "redis.port=${REDIS_PORT}" -p "redis.cluster=true" -P ${REDIS_WORKLOAD} -p status.interval=1 -s  -p \measurementtype=timeseries -p redis.timeout=1000000 -threads ${REDIS_NUMBER_OF_PROCESSES} >> ${YCSB_LOG_FILENAME} 2>&1 &)
fi
