...
cd $YCSB_DIR
ycsbCommand=$(sudo ./ycsb run redis -p "redis.host=${REDIS_HOST}" -p "redis.port=${REDIS_PORT}" -p "redis.cluster=true" -P ${REDIS_WORKLOAD} -p recordcount=${YCSB_RECORDS} -p status.interval=2 -s  -p \measurementtype=timeseries -p redis.timeout=10000 -threads 25 >> ${YCSB_LOG_FILENAME} 2>&1 &)
